...
theRegistry() ->
    ...
    maps:put(9,#inh{name="Susan",children=[3]},M8).
...
descendantsOf(TownR,Visited,[]) ->
	Visited;
descendantsOf(TownR,Visited,Current) ->
	DD = lists:flatten(lists:map(fun (Id) -> childrenOf(Id, TownR) end, Current)),
	New_Current = maps:filter(fun (Id) -> not (lists:member(Id,Visited)) end, DD),
	descendantsOf(TownR,Visited++Current,New_Current).

is_valid(Id,TownR) ->
	has_cycle(TownR,[],[Id]).
is_valid(TownR) ->
	case has_cycle(TownR, [], TownR#inh.children) of
		[] -> true;
		_ -> false
	end.

has_cycle(TownR, Visited, []) ->
	Visited;
has_cycle(TownR, Visited, Current) ->
	DD = lists:flatten(lists:map(fun (Id) -> childrenOf(Id, TownR) end, Current)),
	New_Current = maps:filter(fun (Id) -> (lists:member(Id,Visited)) end, DD),
	has_cycle(TownR,Visited++Current,New_Current).
