...
public class TextSwap {
    ...
    private static Interval[] getIntervals(int numChunks, int chunkSize) {
        ...
        for(int i = 0; i < numChunks; i++){
            intervals[i] = new Interval(i*chunkSize,i*chunkSize+chunkSize);
        } 
        return intervals;
    }
    ...
    private static char[] runSwapper(String content, int chunkSize, int numChunks) {
        ...
        char[] newbuffer = new char[numChunks*chunkSize];
        Thread[] swaps = new Thread[intervals.length];
        int distance = intervals[0].getY() - intervals[0].getX();
        
        for (int i=0; i<intervals.length; i++) {
        	int offset = i * distance;
        	Interval letterInterval = intervals[labels.get(i) - 'a'];
        	swaps[i] = new Thread(new Swapper(letterInterval, content, newbuffer, offset));
        }
        for (Thread swap : swaps) {
        	swap.start();
        }
        for(int i=0; i<numChunks; i++) {
        	try {
