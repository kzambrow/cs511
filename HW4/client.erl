...
do_join(State, Ref, ChatName) ->
	case maps:is_key(ChatName, State#cl_st.con_ch) of
		true -> whereis(list_to_atom(State#cl_st.gui))!{result, self(), Ref, err},
			{err, State};
		false ->
			whereis(server)!{self(), Ref, join, ChatName},
			receive
				{PIDs, Ref, connect, History} ->
					UpdatedState = State#cl_st{con_ch = maps:put(ChatName, PIDs, State#cl_st.con_ch)},
		...
		true ->
			...
			receive
				{_Server, Ref, ack_leave} ->
					UpdatedState = State#cl_st{con_ch = maps:remove(ChatName, State#cl_st.con_ch)},
...
do_new_nick(State, Ref, NewNick) ->
    	SameNick = (State#cl_st.nick == NewNick),
	case SameNick of
		true ->
		...
		false ->
			...
			receive
				...
				{_Server, Ref, ok_nick} ->
					...
					UpdatedState = State#cl_st{nick = NewNick},
					{ok, UpdatedState}
				end
		end.
